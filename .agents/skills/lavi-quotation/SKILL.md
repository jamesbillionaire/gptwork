...
Use `LAVI-QUOTATION-2026.8`, brand `lavi`, on **Legal paper: 8.5 x 14 inches (612 x 1008 pt)**. Both active brands share this paper size but retain separate designs. Start a blank structured job with `python -m quotation_engine.cli new --brand lavi --output work/job.json`; consult `examples/cpsc-01.json` or `examples/cpsc-02.json` for component patterns, not reusable commercial values. Do not select the Lifes Awesome or archived pre-CPSC LAVI profile.
...
Engine 2.2.0 / `LAVI-QUOTATION-2026.8`: title 16 pt; section 11 pt; subsection 10 pt; body/notes 9.5 pt; BOQ/general table 9 pt; metadata/labels 9.5 pt; first-page company heading 9 pt; business contact/footer/page furniture 8 pt; diagram text 9 pt. The 8 pt role is furniture/contact only, not body text.

...
Use `LAVI-QUOTATION-2026.4`, brand `lavi`. Start a blank structured job with `python -m quotation_engine.cli new --brand lavi --output work/job.json`; consult `examples/cpsc-01.json` or `examples/cpsc-02.json` for component patterns, not reusable commercial values. Do not select the Lifes Awesome or archived Legal-size LAVI template.
